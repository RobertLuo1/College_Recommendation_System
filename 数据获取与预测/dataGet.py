...
def trainData(aim_category="文科", aim_province="江苏"):
    ...
    # check(major2017, rank2017)
    # check(major2018, rank2018)
    # check(major2019, rank2019)    

    mydata = []  # 学校 专业 17 18 19分数
    myrank = []  # 位次版
    for i in major2017:
        c_id = i[1]
        ...
        data = [school, i[4], i[6], -1, -1]
        rank_t = [school, i[4], suit(i[6], rank2017), -1, -1]
        mydata.append(data)
    ...
    for i in major2018:
        ...
        if position >= 0:
            ...
            data = [school, i[4], -1, i[6], -1]
            mydata.append(data)
            rank_t = [school, i[4], -1, suit(i[6], rank2018), -1]
            myrank.append(rank_t)
    ...
    for i in major2019:
        ...
        if position >= 0:
            ...
            data = [school, i[4], -1, -1, i[6]]
            mydata.append(data)
            rank_t = [school, i[4], -1, -1, suit(i[6], rank2019)]
            myrank.append(rank_t)
    ...
    conn.close()
    # 分数为-1表示改年分数数据未知
    score = pd.DataFrame(mydata, columns=('学校', '专业', '2017分数', '2018分数', '2019分数'))
    rank = pd.DataFrame(myrank, columns=('学校', '专业', '2017位次', '2018位次', '2019位次'))
    return score, rank
...
def test():
    ...
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()
    x = 'SELECT "Majors"."id", "Majors"."collegeID_id", "Majors"."provinceID_id", "Majors"."categoryID_id", "Majors"."majorName", "Majors"."year", "Majors"."minScore", "Majors"."avgScore", "Majors"."maxScore", "Majors"."firstlevelIDs" FROM "Majors" WHERE ("Majors"."provinceID_id" = 4 AND "Majors"."categoryID_id" = 3)'
    cur.execute(x)
    y = cur.fetchall()
    print(len(y))
    for i in y:
    ...
    print('---------------')
    x = 'SELECT "Majors"."id", "Majors"."collegeID_id", "Majors"."provinceID_id", "Majors"."categoryID_id", "Majors"."majorName", "Majors"."year", "Majors"."minScore", "Majors"."avgScore", "Majors"."maxScore", "Majors"."firstlevelIDs" FROM "Majors" WHERE ("Majors"."provinceID_id" = 4 AND "Majors"."collegeID_id" = 10759)'
    cur.execute(x)
    y = cur.fetchall()
    print(len(y))
    for i in y:
...
if __name__ == "__main__":
    ...
    # f3_s , f3_r = trainData(aim_category="综合")
    # test()
