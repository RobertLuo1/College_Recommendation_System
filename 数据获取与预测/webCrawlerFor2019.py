...
data2019 = []  # 用来存储爬取数据字典的最终列表，最后会被转换为csv保存
...
for i, school in enumerate(school_indices):
    for category in [1, 2]:  # 1：理科 2：文科
        ...
        while True:
            url = f"https://static-data.eol.cn/www/2.0/schoolspecialindex/2019/{school}/32/{category}/7/{page}.json"
            try:
                r = requests.get(url, headers=headers)
            except:
                data2019 = pd.DataFrame(data2019, columns=['学校', '科类', '专业', '最低分'])
                data2019.to_csv("2019数据.csv", encoding="utf-8-sig", index=False)
                exit(1)
